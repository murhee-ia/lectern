...
export async function deleteAccountAction(): Promise<{ error?: string }> {
  ...
  const { data: canDelete, error: guardError } = await supabase.rpc(
  ...
  if (!canDelete) {
    return {
      error:
        "You're the sole Admin of an organization with other members. Hand off the Admin role before deleting your account.",
    };
  }
