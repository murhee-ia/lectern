    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-16">
      {notice === 'invalid_join_code' && (
        <div className="glass-card info-box mb-8">
          <p className="text-sm text-foreground/90">
            The organization code you entered wasn&apos;t valid, so only your
            free personal organization was created. You can join an organization
            later with a valid code, or upgrade to a paid plan to create a new
            one and invite members.
          </p>
        </div>
      )}
      {notice === 'joined_organization' && (
        <div className="glass-card info-box mb-8">
          <p className="text-sm text-foreground/90">
            You&apos;ve joined{' '}
            {joinedOrganization ? (
              <strong>{joinedOrganization.name}</strong>
            ) : (
              'that organization'
            )}
      ...
      <div className="mt-8">
        ...
        <p className="mt-2 text-foreground/70">
          Your workspace is ready — personal and team sessions arrive in a later
          phase.
        </p>
      </div>
    </div>
