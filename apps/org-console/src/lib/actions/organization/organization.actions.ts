...
export async function getOrganizationOverviewAction(
  ...
  if (membershipError) {
    throw new Error(
      `Failed to load the organization: ${membershipError.message}`,
    );
  }
  ...
  if (!membership || !membership.organizations || membership.role !== 'admin')
    return null;
  ...
  if (membersError) {
    throw new Error(
      `Failed to load organization members: ${membersError.message}`,
    );
  }
