...
test("signup with a valid organization code joins that org as Member, in addition to the joiner's own free org-of-one", async ({
  ...
  const { data: ownerCreated, error: createError } = await admin.auth.admin.createUser({
    email: ownerEmail,
  ...
  const { data: ownerOrganizationId, error: ensureError } = await asOwner.rpc("ensure_personal_organization");
  ...
  await expect(page.getByLabel("Organization code (optional)")).toHaveValue(joinCode);
  ...
  const confirmationLink = await getLatestEmailLinkFor(joinerEmail, /http:\/\/127\.0\.0\.1:3000\/auth\/confirm[^"<\s]*/);
  await page.goto(confirmationLink);
  await expect(page).toHaveURL(
    new RegExp(`127\\.0\\.0\\.1:3002/\\?notice=joined_organization&joinedOrganizationId=${ownerOrganizationId}`),
  );
  await expect(page.getByText(organization.name)).toBeVisible();

  // The owner's org now has two members: the original owner (admin) and the joiner (member).
  const { data: ownerOrgMemberships, error: ownerOrgMembershipsError } = await admin
    .from("memberships")
    .select("role")
  ...
  expect(
    joinerMemberships?.some((row) => row.organization_id === ownerOrganizationId && row.role === "member"),
  ).toBe(true);
  expect(
    joinerMemberships?.some((row) => row.organization_id !== ownerOrganizationId && row.role === "admin"),
  ).toBe(true);
...
test("signup with an invalid organization code still completes, falling back to a new org-of-one", async ({ page }) => {
  const email = `join-code-invalid-${Date.now()}@example.com`;

  await page.goto("http://127.0.0.1:3000/signup?join_code=this-code-does-not-exist");
  await page.getByLabel("First name").fill("Test");
  await page.getByLabel("Last name").fill("User");
