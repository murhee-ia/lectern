...
test("unconfirmed account is blocked from password sign-in, resend prompt shown", async ({ page }) => {
  ...
  await page.getByLabel("Email").fill(email);
  await page.getByLabel("Password").fill("correcthorsebattery");
  await expect(page.getByRole("button", { name: "Sign in", exact: true })).toBeEnabled({ timeout: 15_000 });
