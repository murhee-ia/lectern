...
test("email OTP signs in an existing, confirmed user end-to-end via Mailpit", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Send code" })).toBeEnabled({ timeout: 15_000 });
...
test("email OTP rejects an email with no existing account", async ({ page }) => {
  await page.goto("http://127.0.0.1:3000/otp");
  await page.getByLabel("Email").fill(`never-signed-up-${Date.now()}@example.com`);
  await expect(page.getByRole("button", { name: "Send code" })).toBeEnabled({ timeout: 15_000 });
  await page.getByRole("button", { name: "Send code" }).click();
