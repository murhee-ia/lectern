...
test('switching organizations updates the home page and persists across a reload via the selection cookie', async ({
  ...
  const { data: rpcLink, error: rpcLinkError } =
  ...
  const { data: personalOrgId, error: ensureError } = await asUser.rpc(
  ...
  const { data: ownerCreated, error: ownerCreateError } =
    await admin.auth.admin.createUser({
      email: ownerEmail,
      password: 'correcthorsebattery',
      email_confirm: true,
  ...
  const { data: ownerLink, error: ownerLinkError } =
  ...
  const { data: secondOrgId, error: ownerEnsureError } = await asOwner.rpc(
    'ensure_personal_organization',
  );
  ...
  const { error: joinError } = await asUser.rpc('join_organization_by_code', {
    code: secondOrg.join_code,
  });
  if (joinError) throw joinError;

  // --- For real, in the browser ---
  // A fresh generateLink call — the RPC-only link above was already consumed.
  const { data: browserLink, error: browserLinkError } =
    await admin.auth.admin.generateLink({
      type: 'magiclink',
      email,
    });
  ...
  await expect(
    page.getByRole('heading', { name: personalOrg.name, exact: true }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('heading', { name: secondOrg.name, exact: true }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('button', { name: secondOrg.name, exact: true }),
  ).toBeEnabled();
